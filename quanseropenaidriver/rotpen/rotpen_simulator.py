# -*- coding: utf-8 -*-
"""
Created on Sat Feb  6 02:48:50 2021
...
def get_constants(device='rotpen'):
    if device == 'qube':
        # Motor
        Rm = 8.4  # Resistance
        ...
        # Motor
        Rm = 2.6  # Resistance
        kt = 0.0077  # Current-torque (N-m/A)
        km = 0.0077  # Back-emf constant (V-s/rad)

        # Rotary Arm
        mr = 0.2570  # Mass (kg)
        Lr = 0.2159  # Total length (m)
        Jr = mr * Lr ** 2 / 12  # Moment of inertia about pivot (kg-m^2)
        Dr = 0.0024  # Equivalent viscous damping coefficient (N-m-s/rad)

        # Pendulum Link
        mp = 0.1270  # Mass (kg)
        Lp = 0.3365  # Total length (m)
        Jp = mp * Lp ** 2 / 12  # Moment of inertia about pivot (kg-m^2)
        Dp = 0.0024  # Equivalent viscous damping coefficient (N-m-s/rad)
    return [Rm, kt, km, mr, Lr, Jr, Dr, mp, Lp, Jp, Dp]

device = 'rotpen'
[Rm, kt, km, mr, Lr, Jr, Dr, mp, Lp, Jp, Dp] = get_constants(device)
