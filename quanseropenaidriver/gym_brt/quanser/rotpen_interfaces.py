...
from gym_brt.quanser.rotpen_simulator import forward_model_euler, forward_model_ode


class RotpenSimulator(object):
    ...
    def __init__(
        ...
        self.state = (
            np.array([0, 0, 0, 0], dtype=np.float32) + np.random.randn(4) * 0.01
        )
    ...
    def reset_down(self):
        self.state = (
            np.array([0, np.pi, 0, 0], dtype=np.float32) + np.random.randn(4) * 0.01
        )
        return self.state
