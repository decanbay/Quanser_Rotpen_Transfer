# -*- coding: utf-8 -*-
"""
Created on Sun Feb  7 03:23:56 2021
...
from gym_brt.quanser import RotpenSimulator
from gym_brt.envs.rendering import RotpenRenderer


MAX_MOTOR_VOLTAGE = 7
ACT_MAX = np.asarray([MAX_MOTOR_VOLTAGE], dtype=np.float32)
...
class RotpenBaseEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        action = np.clip(np.array(action, dtype=np.float32), -ACT_MAX, ACT_MAX)
        state = self.rotpen.step(action, led=led)
    ...
    def _reset_up(self):
        self.rotpen.reset_up()
        action = np.zeros(shape=self.action_space.shape, dtype=self.action_space.dtype)
        self._step(action)
        return self._get_state()

    def _reset_down(self):
        self.rotpen.reset_down()
        action = np.zeros(shape=self.action_space.shape, dtype=self.action_space.dtype)
        self._step(action)
        return self._get_state()
    ...
    def render(self, mode="human"):
        if self._viewer is None:
            self._viewer = RotpenRenderer(self._theta, self._alpha, self._frequency)
    ...
    def close(self, type=None, value=None, traceback=None):
        ...
        self.rotpen.close(type=type, value=value, traceback=traceback)
